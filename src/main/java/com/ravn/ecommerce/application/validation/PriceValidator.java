 ...
 * Supports multiple numeric types (BigDecimal, Double, Float, etc.).
 */
public class PriceValidator implements ConstraintValidator<ValidPrice, Object> {
    ...
    public boolean isValid(Object value, ConstraintValidatorContext context) {
        ...
        // Convert various numeric types to double for validation
        double price;
        ...
        if (price < min || price > max) {
            ...
            context.buildConstraintViolationWithTemplate(
                    String.format("Price must be between %.2f and %.2f", min, max)).addConstraintViolation();
            return false;
        }
        return true;
    }

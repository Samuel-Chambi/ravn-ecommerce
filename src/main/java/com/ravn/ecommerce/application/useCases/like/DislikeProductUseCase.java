...
public class DislikeProductUseCase implements UseCase<SwitchLikeProductCommand, Void> {
    ...
    public Void execute(SwitchLikeProductCommand command) {
        ...
        log.info("User ID {} unmarking favorite product ID {}", userId, productId);
        ...
        Like like = likeRepository.findByUserIdAndProductId(userId, productId)
                .orElseThrow(() -> new EntityNotFoundException(String.format(
                        "Like for User %d and Product %d does not exist", userId, productId)));
        likeRepository.deleteById(like.getId());
        log.info("Product ID {} disliked successfully", productId);
        return null;
    }
}
